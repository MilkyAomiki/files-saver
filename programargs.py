...
class ProgramInput:
	...
	def __init__(self):
		...
		retrieval_parser = subparsers.add_parser('retrieve', help='Get information form the database', parents=[parent_parser])
		putting_parser = subparsers.add_parser('put', help='Save file to the database', parents=[parent_parser])
		removing_parser = subparsers.add_parser('remove', help='Remove file from the database')
		config_parser = subparsers.add_parser('config', help='Manager the configuration')

		retrieval_parsers = retrieval_parser.add_subparsers()
		retrieval_parsers.add_parser('names', parents=[parent_parser]).add_argument('--names', default=True, action='store_true', help=argparse.SUPPRESS)
		file_retrieval_parser = retrieval_parsers.add_parser('file', parents=[parent_parser])
		file_retrieval_parser.add_argument('--file-name', '-n', type=str, required=True, help='The name of the file to retrieve')
		file_retrieval_parser.add_argument('--directory', '-d', type=str, required=True, dest='fs_dir', metavar='directory', help='Directory to save the file')
		file_retrieval_parser.add_argument('--remove', '-r', action='store_true', default=False, help='Remove file after retrieval')

		putting_parser.add_argument(
			'path', action='extend', nargs='+', metavar='path', type=str, help='The path to the file to back up'
		)
		putting_parser.add_argument('--recursive', '-r', action='store_true', default=False, help='If directory is passed, whether to save files from subdirectories or not')

		removing_parser_group = removing_parser.add_mutually_exclusive_group()
		removing_parser_group.add_argument('--file-name', type=str, help='The name of the file to delete')
		removing_parser_group.add_argument('--all', action='store_true', default=False, help='Remove all files from the database')

		config_subparsers = config_parser.add_subparsers()
		set_config_parsers = config_subparsers.add_parser('set', help='Change configuration').add_subparsers()

		set_config_cnn_parsers = set_config_parsers.add_parser('connection')
		set_config_cnn_parsers.add_argument('connection', type=str, help = 'Connections string')
